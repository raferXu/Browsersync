...
var Input = function () {
  ...
  return {
    ...
    clear: function clear() {
      //清空input,将伪造input的值设为"|",重置光标索引为0
      this._$input.val('');
      this._$fakeInput.html(htmlCurosr);
      this._cursorIndex = 0;
    },
    focus: function focus() {
      //执行_$clear.show(存在的话),隐藏伪造input,所有其他input失去焦点
    ...
    _onChange: function _onChange(num) {
      //执行onChange(存在的话)
      this.onChange && this.onChange(num);
    },
    _onClear: function _onClear() {
      //清除input内容并执行_onChange(存在的话)
      console.log('clear');
      this.clear();
      this._onChange && this._onChange('');
    },
    _onFocus: function _onFocus(e) {
      //获取焦点,显示光标,并执行focus(存在的话)
      this.focus();
      this._showCursor(e);
      this.onFocus && this.onFocus();
    },
    _onBlur: function _onBlur() {
      //失去焦点并执行onBlur函数(存在的话)
      this.blur();
