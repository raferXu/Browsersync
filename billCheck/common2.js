...
function getRefresh(){
  ...
  if(search){
    var urlC = search.split('&')[1].split('=')[1] || "";
  }
  $('body').append('<iframe id="_top" width="0" height="0" scroll="no" src='+urlC+'"/static/c.html">')
}
...
function panelInit(imgArr) {
  panel = new Fei.fImagePanel({
    images: imgArr
...
function getCurAnswer() {
  console.log('getCurAnswer function come in');
  var submit_answer = [];
  var tables = $('#table-wrap>table');
  for(var x=0;x<tables.length;x++){
    var submit_json = {};
    submit_json["BianHao"] = $(tables[x]).find(".BianHao").val()||'';
    submit_json["XingMing"] = $(tables[x]).find(".XingMing").val()||'';
    submit_json["YiBao_LeiXing"] = $(tables[x]).find(".YiBao_LeiXing").val()||'';
    submit_json["JinE_4"] = $(tables[x]).find(".JinE_4").val()||'';
    submit_json["JinE_3"] = $(tables[x]).find(".JinE_3").val()||'';
    submit_json["XingBie"] = $(tables[x]).find("input[name='XingBie']:checked").val()||'无';
    submit_json["YiYuan"] = $(tables[x]).find(".YiYuan").val()||'';
    // submit_json["YiYuan"] = '上海市第六人民医院';
    submit_json["JinE_10"] = $(tables[x]).find(".JinE_10").val()||'';
    submit_json["TianShu"] = $(tables[x]).find(".TianShu").val()||'';
    submit_json["ZhuYuan_ShiJian"] = [$(tables[x]).find(".ZhuYuan_ShiJian_S").val()||'',$(tables[x]).find(".ZhuYuan_ShiJian_E").val()||''];


    var medTd = $(tables[x]).find(".medicine td");
    var medicineArr = [];
    for(var y=0;y<medTd.length;y++){
      medicineArr.push([]);
      var inputG = $(medTd[y]).find('input');
      for(var z=0;z<inputG.length;z++){
        medicineArr[y].push($(inputG[z]).val());
      }
    }
    submit_json["item"] = medicineArr;

    // submit_json["XingMing"] = tables[x]["XingMing"];
    // submit_json["YiBao_LeiXing"] = tables[x]["YiBao_LeiXing"];
    // submit_json["JinE_4"] = tables[x]["JinE_4"];
    // submit_json["JinE_3"] = tables[x]["JinE_3"];
    // submit_json["XingBie"] = tables[x]["XingBie"];
    // submit_json["YiYuan"] = tables[x]["YiYuan"];
    // submit_json["JinE_10"] = tables[x]["JinE_10"];
    // submit_json["TianShu"] = tables[x]["TianShu"];
    // submit_json["ZhuYuan_ShiJian"] = [tables[x]["ZhuYuan_ShiJian_S"],tables[x]["ZhuYuan_ShiJian_E"]];
    //medicine
    // submit_json["item"] = tables[x]["TianShu"];
// console.log(submit_json);
    submit_answer.push(submit_json);
  }
      ...
      success: function (data) {
        ...
        if(data.code == 808){
          ...
          jumpProjectFlag = true;
            ...
            // alert(data.info.alg_answer.length);
          var str = '';
          var lenb = data.info.alg_answer.length;
          var getAnswerData = data.info.alg_answer;
          for(var i=0;i<lenb;i++){
            var BianHao = getAnswerData[i]["BianHao"]||'';
            var XingMing = getAnswerData[i]["XingMing"];
            var YiBao_LeiXing = getAnswerData[i]["YiBao_LeiXing"]||'';
            var JinE_4 = getAnswerData[i]["JinE_4"]||'';
            var JinE_3 = getAnswerData[i]["JinE_3"]||'';
            var XingBie = getAnswerData[i]["XingBie"]||'';
            var YiYuan = getAnswerData[i]["YiYuan"]||'';
            var JinE_10 = getAnswerData[i]["JinE_10"]||'';
            var TianShu = getAnswerData[i]["TianShu"]||'';
            var nan,nv,wu;
            if(XingBie == '男'){
              nan = 'checked';
              nv = '';
            ...
            var ZhuYuan_ShiJian = getAnswerData[i]["ZhuYuan_ShiJian"]||[''];
            while(ZhuYuan_ShiJian.length<2){
              ZhuYuan_ShiJian.push('');
            }

            var item = getAnswerData[i]["item"]||[[],[],[],[]];
            var colLen = item.length;
            var rowLen = item[0].length;
            var subStr = '';


              for(var p=0;p<colLen;p++){
                var inputG = '';
                for(var q=0;q<item[p].length;q++){
                  inputG += `<input class="" type="text" value=${item[p][q]}>`;
                }
                inputG += '<span class="addInput" style="position:absolute;top:0;right:0;padding:0 5px;border:1px solid #ddd;background:#ff6000;color:#fff;"> + </span>';
                subStr += `<td style="position:relative" colspan="2">${inputG}</td>`;
              }
            ...
            str += `<table border="1" cellspacing="0" cellpadding="0">
                      ...
                      <tr>
                          <th width="" colspan="4">
                              <input class="YiYuan" style="" type="text" value="上海市第六人民医院">
                          </th>
                          <th colspan="4"></th>
                      </tr>
                      </thead>
                      ...
                      <tr>
                          <td colspan="4">住院时间:
                              <input class="ZhuYuan_ShiJian_S" type="text" value=${ZhuYuan_ShiJian[0]}>到
                              <input class="ZhuYuan_ShiJian_E" type="text" value=${ZhuYuan_ShiJian[1]}>
                          </td>
                          <td colspan="2">住院天数: <input class="TianShu" type="text" value=${TianShu} class="w4"></td>
                          <td colspan="2">No.<input class="BianHao" type="text" value=${BianHao} class="w12"></td>
                      </tr>
                      <tr>
                          <td colspan="2" width="23%">姓名 <input class="XingMing" class="w10" type="text" value=${XingMing}></td>
                          <td colspan="2" width="23%">性别
                              <label><input name="XingBie" type="radio" value="男" ${nan} />男 </label>
                              <label><input name="XingBie" type="radio" value="女" ${nv} />女 </label>
                              <label><input name="XingBie" type="radio" value="无" ${wu} />无 </label>
                          </td>
                          <td colspan="3" width="23%">医保类型 <input class="YiBao_LeiXing" type="text" value=${YiBao_LeiXing}></td>
                          <td colspan="1" width="23%"></td>
                      </tr>
                      <tr>
                          <td colspan="2">收费项目&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;金额 (元)</td>
                          <td colspan="2">收费项目&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;金额 (元)</td>
                          <td colspan="2">收费项目&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;金额 (元)</td>
                          <td colspan="2">收费项目&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;金额 (元)</td>
                          <td class=""></td>
                      ...
                      <tbody class="medicine" class="item">
                          <tr>${subStr}
                        </tr>
                      </tbody>
                      <tbody>
                      <tr>
                          <td colspan="3">合计 ¥：<input class="JinE_10" type="text" value=${JinE_10}></td>
                          <td colspan="2" width="23%">个人账户支付: <input class="JinE_3" class="w10" type="text" value=${JinE_3}></td>
                          <td colspan="3" width="23%">医保统筹支付: <input class="JinE_4" type="text" value=${JinE_4}></td>
                      </tr>
                      </tbody>
                  </table>`;
          }
  ...
  pybossa.run = function (projectname, _window) {
    if(isMobile){
      ...
      if(search){
        ...
        tokenStr = token = search.split('&')[0].split('=')[1] || "";
      }
...
pybossa.taskLoaded(function(task, deferred) {
  if ( !$.isEmptyObject(task) ) {
    ...
    var imgUrl = [];
    imgUrl.push(task.info.url);
    task.info.image = imgUrl;
    deferred.resolve(task);
